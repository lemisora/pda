...
class StorageManager:
    def __init__(self, threshold: int):
        ...
        self.metadata : Dict[str, str] = {} # Estructura esperada 'nombre_archivo' e 'IP_Worker'
        self.nodes : Dict[str, Dict] = {}   # Estructura esperada 'IP_Worker' y 'detalles_nodo'
    ...
    def add_file_record(self, filename: str, worker_ip: str):
        with self.lock:
            ...
            if worker_ip in self.nodes:
                ...
                self.nodes[worker_ip]["last_update"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
...
def start_master(host='0.0.0.0', port=5000):
    ...
    try:
        ...
        while True:
            ...
            threading.Thread(target=handle_client, args=(conn, addr, manager), daemon=True).start()
